...
**Beliefs** are durable, evolving interpretations of how the business works: "B2B growth is treated as higher-quality growth because it carries partnership leverage." "Marketing leverage matters only if bookings growth holds." "Forecast misses concentrate in the shoulder quarters." Beliefs are the judgment layer, and a belief is not a single observation restated in a confident voice. It is a *synthesis*: two or more curated concepts weighed against each other to produce a conclusion none of them states alone. A flat-margin concept and an expanding-margin concept, read together, tell you a margin beat belongs to one segment and not the other — a judgment neither concept makes by itself. If a candidate belief just repeats one concept in longer language, it is not a belief yet. Each belief is stored as a structured object carrying a claim that names both the pattern and the action it implies, a self-contained explanation built from real quotes and numbers, the concepts it was synthesized from, a confidence level, the task it serves, an owner, and a status - active, weakening, or retired. That structure makes beliefs auditable, which makes them trustworthy, which makes them usable in rooms where a wrong answer is a career event.

Beliefs organize into streams, but the streams should be derived from the tasks the system must serve, not slotted into a fixed taxonomy decided in advance. In a finance setting they often land on domains like business performance, forecast reliability, leadership narrative, metric interpretation, source trust, investor relations, and variance explanation - but those are what tends to emerge, not a checklist to fill. If a task the system must serve has no stream behind it and the documents cannot yet produce one, that gap should be named, not papered over. A stream is not a folder of summaries; it is a maintained position that new evidence continuously confirms, weakens, or overturns.
...
**Judgment sits in the belief store, retrieved by relevance.** Beliefs are queried at reasoning time, scoped to the question's domain. An agent answering a margin question loads active beliefs from the performance and variance streams - not the whole store. Beliefs enter the context *with their metadata*: claim, confidence, source concepts, status. The agent is instructed to treat them as strong priors, not ground truth, and to cite them when they shape an answer.
...
The answer is a promotion pipeline with explicit criteria. Extracted claims enter as **observations** - timestamped, source-linked, carrying no interpretive authority. An observation becomes a **concept** only when it recurs: the same interpretive pattern appearing across multiple independent artifacts, typically spanning at least two or three quarters, from more than one author or forum. A concept becomes part of an **active belief** only when it combines with at least one other concept to produce a conclusion neither states alone, survives a contradiction check against the existing store and existing data, and is ratified by a human reviewer. The criteria that matter are recurrence (does this pattern repeat?), independence (does it appear outside a single author's deck?), specificity (is it falsifiable, or is it a platitude like "growth is good"?), consequence (would an agent answer differently if this belief were true versus false?), and synthesis (does it say something no single concept already says?). A claim that fails the falsifiability or consequence test is not a belief; it is filler. A claim that passes them but merely restates one concept is not a belief either; it is a concept wearing a confident voice. Admitting either is how the judgment layer dies of dilution.

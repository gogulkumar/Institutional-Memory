...
This applies to every `definition` in Layer 3 and every `claim` in Layer 4 — the two places a human actually reads first. Two failure modes to avoid, both learned the hard way:

**Failure mode 1: cramming the evidence into the definition.** A definition like "A recurring phrase family that reframes flat or declining segment margin as a deliberate strategic choice rather than a shortfall" is trying to state the fact *and* argue for why it's a pattern in the same sentence — which makes it dense and hard to parse. Fix: state only what the thing *is* in the `definition`/`claim` field, in one plain sentence. Push "how many quarters," "which exact quotes," "how confident to be" into the separate fields that already exist for that (`evidence`, `confidence`, `pattern_observed`/`Evolution Trail`). Use the read-it-out-loud test: if you wouldn't say the sentence that way to a colleague across a table, rewrite it. Compare:
...
**Failure mode 3: going plain and neutral all the way to generic.** Fixing failure modes 1 and 2 has an obvious overcorrection: stripping out *all* specifics in the process, leaving a `claim` that's easy to read but says nothing you couldn't have guessed. "Whenever discounting is mentioned, management specifies that suppliers or partners are funding it, not [the company]'s own margin" is plain and neutral — and also empty, because it has no numbers, no quotes, no quarters. The actual finding disappears. Fix: a `claim` should be readable as one or two plain sentences *and* contain the real quotes and figures inline, not pushed off into a separate evidence field the reader has to go dig up. Compare:
- Too generic (plain but empty): *"Whenever discounting is mentioned, management specifies that suppliers or partners are funding it, not [the company]'s own margin — and the scale of this keeps growing each quarter it's mentioned."*
- Plain and specific: *"Supplier-funded discounting went from '20%+ of promotional spend cost-shared with suppliers' (Q3'25) to '30% of Q4 promotional spend, up 10+ points from Q3' (Q4'25) to '25% more vendors enrolled in the co-op program' (Q1'26) — the scale keeps growing, and every time, it's framed as the supplier's money, not [the company]'s own margin."*

The test for whether a `claim` has failed this way: could someone who has *not* read the source documents tell you, from the claim alone, what was actually said and roughly when? If the answer is no — if they'd have to go open the evidence field to find out what the pattern even consists of — the claim has been simplified past the point of being useful. Plain language is about sentence *structure* (short, spoken, no jargon), not about deleting content.

**This applies more strictly to Layer 4's `claim` than to Layer 3's `definition` — and for a second reason beyond just being the layer's output.** A Layer 3 `definition` naming what a pattern *is* can stay general, because its evidence sits in the same record, one field away (`evidence`, `pattern_observed`) — a reader is one glance from the specifics. A Layer 4 `claim` cannot lean on that same nearby field the same way, because a belief's `explanation` isn't a citation list — it has to make the case for the claim using its own quotes and numbers, standalone, without narrating which upstream concepts it drew from (see "Belief Streams" above). So the `claim` line has to carry the real quotes and numbers itself, and so does the belief's **heading** — a heading like "Promotions are described as funded by partners" tells a reader nothing they'd have to open the record to get; the heading itself should carry the real quote or number *and* the resulting directive, since it's the first thing anyone scanning the file actually reads and, for a belief specifically, the only thing some downstream consumers will ever load.

**Failure mode 4: repeating the company's own claim back as if it were a finding.** Even a specific, quote-filled statement can still fail if it just restates management's own promotional framing without adding any analytical distance — e.g. "discounting is described as funded by suppliers, not by [the company]" sounds like a fact because it has a specific quote behind it, but it's actually just repeating what management *said*, in the same warm register they said it in. The reviews rarely give you enough to verify a claim like that (no cost-allocation detail shown, no supplier-side numbers) — so the belief should say what was claimed, note that it's asserted rather than demonstrated, and name what it would take to actually check it. Compare:
- Repeats the claim: *"Supplier-funded discounting grew from 20%+ to 30%+ of promotional spend — always the supplier's money, never [the company]'s margin."*
- Adds analytical distance: *"Discount cost-sharing grew from 20%+ to 30%+ of promotional spend in one quarter, and management calls it 'supplier-funded' every time — but none of the three reviews show the cost-allocation detail that would let you verify who's actually absorbing it."*
...
| 4. Belief Streams | Weigh multiple business-context concepts against each other and state a goal-directed claim none of them makes alone (judgment) | "The margin beat must be attributed to Consumer efficiency, not B2B scale, because B2B's flat margin is deliberate reinvestment while Consumer's comes from spend leverage" |
...
Each approved business context record should carry: `concept_id`, `concept_name`, `concept_type`, `definition`, `business_interpretation`, `positive_signal`, `negative_signal`, `related_metrics`, `related_concepts`, `supporting_evidence`, `contradicting_evidence`, `confidence`, `scope`, `owner`, `last_reviewed`, `review_trigger`, `agent_reuse`, and `example`. A record with no clear `agent_reuse` isn't pulling its weight. Full field descriptions and a worked example (Marketing Efficiency) are in `references/schemas.md`.

- **`agent_reuse`** — one or two sentences stating concretely what an agent should *do* with this record (watch for it, use it as a template, treat it skeptically), not just what it means.
- **`example`** — a single, specific, fully worked-out instance pulled from the actual source material, written out in enough detail that someone reading only this field understands the concept without needing the `definition` or `evidence` fields at all. This is not the same as `supporting_evidence`, which is a list of citations pointing back at the source — `example` is one of those instances, told as a short, self-contained story: what quarter, what was actually said or done, and why that instance in particular illustrates the concept. If a record's `example` could apply equally well to a different concept, it's too generic and needs to be replaced with something more specifically tied to this concept alone.
...
A belief is not a business-context concept restated in a different tone. A concept documents one pattern with its own evidence; a belief only earns its place by weighing *multiple* concepts against each other and stating a conclusion none of them makes individually — the kind of thing that only becomes visible once the concepts are read together, not one at a time. If a candidate belief just repeats one concept's `definition` in longer or more confident language, it isn't a belief yet; either find what it produces when combined with something else, or leave it as a concept.

**A belief must be goal-derived, not assumed as a fixed taxonomy in advance.** Don't start from a generic checklist of belief "categories" (voice, risk, narrative, etc.) and slot findings into it. Start from the actual downstream task(s) the belief memory has to serve (e.g., generating a script, evaluating a draft, benchmarking against a peer, deciding what to focus on next), ask what each task actually needs to check or produce, and let the belief categories fall out of that. If a task has no belief serving it and the corpus genuinely can't produce one (e.g., benchmarking needs a competitor's data and none exists), say so explicitly as a named gap — don't leave it silently empty and don't force a low-confidence concept to fill it.

**Every belief's claim must state its own action, not just its observation.** The heading and opening statement should read as one direct, declarative sentence that both names the pattern *and* says what a downstream task must do because of it — reproduce it, vary it, never fabricate it, or treat it as a signal. Don't separate "here's the pattern" from "here's what to do about it" into two places (a claim plus a trailing "what this means" note) — fold them into one sentence. No first person, no unresolved absolutist language ("always," "never," "zero") unless the evidence genuinely supports zero exceptions.

**A belief must be fully self-contained — it must not narrate the extraction process that produced it.** The explanation under a belief's claim should read as a direct case for the belief itself: real quotes, real numbers, real quarters, stated as evidence for the claim. It must not talk about the underlying concepts as objects ("concept A says X, concept B says Y, combined they say Z") — that is process narration, not explanation, and it makes the belief unreadable without also having the concept file open. A short `source_concepts` tag under the heading (for traceability back to the business-context records) is fine and expected — but it stays a one-line pointer, separate from the prose, never woven into it.

**A belief must be falsifiable and honestly confident.** State what would prove it wrong, and don't round up a single-instance or low-track-record concept into a confident-sounding belief — bundle genuinely thin findings together and label them as low-confidence rather than dressing any one of them up alone. A concept that's real but doesn't combine with anything else to produce a further conclusion should stay a concept, explicitly excluded and named as such, not force-promoted for the sake of coverage.

Each record: `belief_id`, `claim` (the direct, action-stating sentence — this is the field most likely to be read alone), `explanation` (self-contained, evidence-first, no concept-ID narration), `source_concepts` (traceability only, ≥2 entries), `confidence`, `serves_task` (which downstream task(s) this belief actually feeds), `owner`, `status`. When new evidence arrives, update the belief — don't just overwrite it; state whether it's confirmed, weakened, strengthened, or retired, and why. Template + a worked update example in `references/schemas.md`.
...
11. Derive belief categories from the downstream task(s) this memory has to serve, then synthesize beliefs by weighing multiple approved concepts against each other — not by restating concepts one at a time. Name any task a belief can't yet serve as an explicit gap rather than leaving it silently unaddressed.
...
- `references/schemas.md` — full JSON templates for artifact records, extracted facts, business context records, evidence ledger entries, and belief records, plus a worked end-to-end example (B2B growth) and a glossary. Read this when actually drafting records rather than just discussing the framework.

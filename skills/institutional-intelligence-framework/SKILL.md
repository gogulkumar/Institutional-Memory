...
A belief stream is the *current* durable interpretation of how part of the business works, tracked as evidence accumulates — distinct from a static business-context definition. Business context says "marketing efficiency is positive only when growth stays healthy" (the rule). A belief stream says "recent evidence supports marketing leverage as a margin driver, but confidence should drop if demand softens" (the current read) — but only once that read has been checked against everything else known to bear on it, not lifted from the marketing-efficiency concept alone.

**A belief must be synthesized from multiple concepts weighed against each other — never one concept in, one belief out, just reworded.** Restating a single business-context concept in belief language is translation, not synthesis, and it is not yet a belief. A real belief should only exist if it survives being checked against the other concepts that bear on the same question: some concepts will turn out to support the same belief and reinforce it, some will cut against each other and lower confidence or block the belief entirely, and some concepts are real and evidenced but too narrow or one-off to rise to the level of a belief on their own — they stay concepts, not every concept graduates. Before writing a belief, list which concepts are being weighed together and what each one contributes; if that list has exactly one entry, the belief isn't ready yet — go find what else bears on it first, or hold it as a candidate.

Typical streams: Business Performance, Forecast Reliability, Leadership Narrative, Metric Definition, Source Trust, Variance Explanation, Investor Relations, Risk. Each record: `belief_stream`, `current_belief`, `source_concepts`, `supporting_evidence`, `contradicting_evidence`, `confidence`, `watch_item`. `source_concepts` is the list of `concept_id`s actually weighed together to reach this belief — it's what makes the synthesis rule above checkable rather than a promise; a belief with only one entry there should be treated as not yet earned. When new evidence arrives, update the belief — don't just overwrite it; state whether it's confirmed, weakened, strengthened, or retired, and why. Template + a worked update example in `references/schemas.md`.

...
**This applies more strictly to Layer 4's `claim` than to Layer 3's `definition`.** A Layer 3 `definition` naming what a pattern *is* can stay general, because its evidence sits in the same record, one field away (`evidence`, `pattern_observed`) — a reader is one glance from the specifics. A Layer 4 `claim` doesn't get that same shortcut just because Layer 4 also has an `explanation` field: `explanation` carries the *reasoning* for why the claim holds, not a place to defer the *evidence* the claim itself needs to be credible — a reader scanning belief memory reads the `claim` as the finding, not `explanation`, so the real quotes and numbers still have to live in the `claim` line itself. This also applies to the belief's **heading**, not just its `claim` body — a heading like "Promotions are described as funded by partners" tells a reader nothing they'd have to open the record to get; the heading itself should carry the real quote or number, since it's the first thing anyone scanning the file actually reads.
...
| 4. Belief Streams | Combine ≥2 concepts into a claim none of them makes alone (judgment) | "Consumer margin keeps expanding while B2B stays flat-to-down every quarter — read together, that's a deliberate segment-priority choice, not a Consumer-specific strength or a B2B-specific problem" |
...
A belief is a durable interpretation of how part of the business works, built by combining multiple approved business-context concepts — never a single concept restated in belief language.

**Hard rule: a belief must combine at least 2 concepts and produce a conclusion that none of them makes alone.** Restating a single business-context concept in belief language is translation, not synthesis, and it is not yet a belief. A real belief should only exist if it survives being checked against the other concepts that bear on the same question: some concepts will turn out to support the same belief and reinforce it, some will cut against each other and lower confidence or block the belief entirely, and some concepts are real and evidenced but too narrow or one-off to rise to the level of a belief on their own — a concept that doesn't combine with anything stays a concept; it does not get promoted just because it's true and evidenced. Before writing a belief, list which concepts are being weighed together and what each one contributes; if that list has exactly one entry, the belief isn't ready yet — go find what else bears on it first, or hold it as a candidate.

**Derive belief categories from downstream tasks, not from a fixed checklist.** Don't start belief-building from a preset list of category names — derive the categories a given memory actually needs from the real downstream tasks named in Step 0.5 (generation, evaluation, benchmarking, focus-setting, or whatever the corpus was scoped for). If the corpus can't yet serve a task a category would need, name that explicitly as an unfilled gap rather than leaving the category silently empty or forcing a thin belief to fill it. Typical categories that have shown up in practice — Business Performance, Forecast Reliability, Leadership Narrative, Metric Definition, Source Trust, Variance Explanation, Investor Relations, Risk — are illustration, not a checklist to complete.

**Record fields:** `belief_id`, `claim`, `explanation`, `source_concepts`, `confidence`, `serves_task`, `owner`, `status`.
- `claim` — one direct sentence naming the pattern *and* the action a downstream task must take from it — no separate "what this means" appendix; the action belongs in the same sentence as the pattern. See the Writing rule above — this is the field it's about.
- `explanation` — the reasoning the claim rests on, in the writer's own analysis. It must **not** narrate the extraction process ("concept A says X, concept B says Y, so...") — that's process narration, not explanation, and it's explicitly forbidden here. Explain why the combination matters, not which concepts were consulted.
- `source_concepts` — the traceability pointer: the `concept_id`s (≥2, per the hard rule above) actually combined to reach this belief. It stays a one-line list — it never gets woven into `explanation`'s prose. A record with fewer than 2 entries is a candidate, not an active belief.
...
- `owner`, `status` — accountability and lifecycle (e.g. active / weakening / retired).

When new evidence arrives, update the belief — don't just overwrite it; state whether it's confirmed, weakened, strengthened, or retired, and why. Template + a worked example in `references/schemas.md`.
...
11. Create beliefs by combining ≥2 approved concepts into a claim that names the pattern and the action a task must take from it — never promote a single concept into belief language, and name any task the corpus can't yet serve as an explicit gap rather than leaving it empty.
